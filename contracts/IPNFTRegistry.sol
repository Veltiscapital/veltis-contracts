 ...
 * with enhanced security, recovery mechanisms, and MEV protection
 */
contract IPNFTRegistry is ERC721URIStorage, ERC721Enumerable, AccessControl, IERC2981, ReentrancyGuard, Pausable {
    ...
    // Transaction fees
    uint256 public transactionFeePercentage = 250; // 2.5% in basis points
    ...
    // Events
    event IPNFTMinted(uint256 tokenId, address owner, string title);
    ...
    /**
     * @dev Mint a new IP-NFT with enhanced security and MEV protection
    ...
    function mintIP(
    ...
    ) public nonReentrant whenNotPaused notBlocked(recipient) preventMEV returns (uint256) {
        ...
        emit IPNFTMinted(newTokenId, recipient, title);
     ...
     */
    function setTransactionFeePercentage(uint256 percentage) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Must have admin role");
